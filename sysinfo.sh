...
case "$os_name" in
	Linux)
		...
		[ -n "$load_avg" ] && \
			...
			print_info "load average" "unavailable"
		lsblk_out=$(lsblk 2>/dev/null)
		[ -n "$lsblk_out" ] && \
			print_info "block devices" "see below"
		echo "$lsblk_out" | \
		...
		[ -n "$interrupts" ] && \
			print_info "interrupts" "see below"
		echo "$interrupts" | \
			while IFS= read -r line; do
				printf "\t%s\n" "$line"
			done
		...
		[ -n "$pci_devices" ] && \
			print_info "PCI devices" "see below"
		echo "$pci_devices" | \
			while IFS= read -r line; do
				printf "\t%s\n" "$line"
			done
	...
	FreeBSD|OpenBSD)
		...
		[ -n "$geom_disk" ] && \
			print_info "geom disks" "see below"
		echo "$geom_disk" | \
			while IFS= read -r line; do
				printf "\t%s\n" "$line"
			done
