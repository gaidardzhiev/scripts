...
case "${1}" in
	...
	m2m)
		...
		[ -z "${current}" ] && \
			...
			exit 1
		[ "${current}" = "main" ] && \
			printf "you are already on main branch. Nothing to merge.\n" && \
			exit 1
		printf "checking out main branch...\n"
		git checkout main || \
			{ printf "failed to checkout main branch\n"; exit 1; }
		printf "merging branch '%s' into main...\n" "${current}"
		git merge --no-ff "${current}" || \
			{ printf "merge failed... resolve conflicts manually...\n"; exit 1; }
